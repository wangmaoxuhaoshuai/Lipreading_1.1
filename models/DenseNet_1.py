...
class LipNet_1(nn.Module):
    def __init__(self):
        super(LipNet_1, self).__init__()
        ...
        self.l2 = nn.Sequential(CBR(64, 128, stride=(2, 2, 2)),
                                # Bottleneck(32),
                                Block4(128),
                                CBR(128, 128),
                                # CBR(32, 32)
                                )
        self.l3 = nn.Sequential(CBR(128, 256, stride=(2, 2, 2)),
                                # Bottleneck(64),
                                Block4(256),
                                CBR(256, 256),
                                # CBR(64, 64)
                                )
        self.l4 = nn.Sequential(CBR(256, 256, stride=(1, 2, 2)),
                                # Block3(256),
                                Block4(256),
                                CBR(256, 512),
                                # CBR(128, 128)
        ...
        # Rnn
        self.gru1 = nn.GRU(3 * 14 * 14, 256, bidirectional=True, batch_first=True)
        self.gru2 = nn.GRU(512, 256, bidirectional=True, batch_first=True)

        self.fc = nn.Sequential(nn.Dropout(0.5),
                                nn.LeakyReLU(0.1),
                                nn.Linear(256 * 512, 311)
                                )
